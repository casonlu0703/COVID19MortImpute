...
rm(list = ls())
# memory.limit(size=30000) # for windows system
...
fit_hurdle <- readRDS(paste0("impute/results/fit_hurdle_agg", i,"_new.RDS"))


g_stan_dig <- stan_diag(fit_hurdle)
ggarrange(g_stan_dig[[1]], g_stan_dig[[2]], g_stan_dig[[3]], nrow = 3)
ggsave(paste0("impute/results/stan_diag", i,"_new.png"),
...
saveRDS(sum_estimates, paste0("impute/results/sum_estimates_hurdle_agg", i,"_new.RDS"))

traceplot(fit_hurdle, pars = pars)
ggsave(paste0("impute/results/traceplot of fixed effects_hurdle_agg", i,"_new.png"), device = "png",
       height = 12, width = 14)
...
write.csv(diag_stats, paste0("impute/results/diag_stats", i,"_new.csv"), row.names = F)


#### Sample data

ix_ymis <- grep("Ymi", names(fit_hurdle@sim$samples[[1]]))
ymis_draws <- lapply(ix_ymis, function(x) {
...
covid19deaths_dist <- lapply(c(1:nrow(ymis_draws)), function(x) {
  tmp_y <- ymis_draws[x, ]
...
ggsave(paste0("impute/results/impute distribution_hurdle_agg", i,"_new.tiff"),
...
ggsave(paste0("impute/results/impute distribution_age_hurdle_agg", i,"_new.tiff"),
...
ysim_draws <- lapply(ix_ysim, function(x) {
  y <- c(fit_hurdle@sim$samples[[1]][[x]][warmup:fit_hurdle@sim$iter],
         fit_hurdle@sim$samples[[2]][[x]][warmup:fit_hurdle@sim$iter],
         fit_hurdle@sim$samples[[3]][[x]][warmup:fit_hurdle@sim$iter])
  round(y)
...
ysim_draws <- lapply(ysim_draws, function(x) {
  x[x_samp]
})
...
covid19deaths_dist <- lapply(c(1:nrow(ysim_draws)), function(x) {
  tmp_y <- ysim_draws[x, ]
...
covid19deaths_sum[, model := ifelse(i == 8, "M1",
                                    ifelse( i == 16, "M2",
                                            ifelse(i == 18, "M3", "NA")))]
saveRDS(covid19deaths_sum, paste0("impute/results/sim_data_dt", i, "_new.RDS"))
...
saveRDS(g3, paste0("impute/results/sim_data_plot", i, "_new.RDS"))

ggsave(paste0("impute/results/distribution simulated and data prop hurdle_agg", i,"_new.tiff"),
       plot = g3, device = "tiff", height = 16, width = 14, dpi = 300,
       units="in", compression = "lzw", type = "cairo")
