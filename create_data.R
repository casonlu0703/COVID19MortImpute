...
covid19d_cty_old <- get_covid_death_cty(version = "old")
mort2020 <- get_mort_nation_state()
mort2020_old <- get_mort_nation_state(version = "old")
...
# FIPS 2270 (Wade Hampton Census Area, AK) and and 46113 (Shannon County, SD)
# are not found in the census (county_pop) data
covid19d_cty_old[, `:=` (county = NULL, state = NULL)]
covid19d_cty_old <- merge(covid19d_cty_old, county_pop, by = c("fips", "age_group"), all.x = T)
# removing the two FIPS that had no population information
covid19d_cty_old <- covid19d_cty_old[!fips %in% c(2270, 46113)]
covid19d_cty_old[, row_ix := c(1:.N)]
covid19d_cty_old <- merge(covid19d_cty_old, uninsure, by = c("fips"), all.x = T)
covid19d_cty_old <- covid19d_cty_old[order(row_ix)]
...
covid19d_cty_old <- merge(covid19d_cty_old, unique(policy[, .(fips, svi_cate, svi_num)]), by = "fips", all.x = T)
covid19d_cty_old <- covid19d_cty_old[order(row_ix)]
